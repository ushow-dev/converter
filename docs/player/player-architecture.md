...
Для снижения нагрузки на CDN/origin используется **p2p-media-loader** (Novage, MIT license).

### Принцип работы
- Зрители одного контента объединяются в swarm через WebTorrent tracker
- Первый зритель получает `.ts` сегменты с CDN, последующие — частично от peers через WebRTC
- При недоступности WebRTC — graceful fallback на 100% HTTP

### Компоненты
- **p2p-media-loader-hlsjs** — mixin для hls.js, перехватывает загрузку сегментов
- **wt-tracker** — WebTorrent tracker (Docker-контейнер на API-сервере), `wss://t.pimor.online`
- **P2P метрики** — клиент (p2pMetrics.ts) отправляет статистику на `POST /api/player/p2p-metrics`, API экспортирует в Prometheus

### Feature flag
```
NEXT_PUBLIC_P2P_ENABLED=true|false
NEXT_PUBLIC_P2P_TRACKER_URL=wss://t.pimor.online
```

### Мониторинг
Grafana dashboard "P2P HLS Overview" (`infra/grafana/provisioning/dashboards/p2p-overview.json`):
- P2P Ratio % — доля трафика через peers
- Bandwidth Saved — байты, не ушедшие на origin
...
| p2p-media-loader (не CDNBye/Peer5) | Полностью open source, self-hosted tracker, нет лимитов |
